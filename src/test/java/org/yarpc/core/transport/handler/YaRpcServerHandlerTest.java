...
class YaRpcServerHandlerTest {
    ...
    void setUp() {
        ch = new EmbeddedChannel(
                new ProtocolDecoder(new KryoSerializer()),
                new ProtocolEncoder(new KryoSerializer()),
                new YaRpcServerHandler(new HelloImpl()));
    }

    @Test
    void writeFailedResponse() throws NoSuchMethodException, InterruptedException {
        ch.writeInbound(new Request(
                1L,
                Hello.class,
        ...
        Assertions.assertThat(resp.getE().toException())
